...
public class rdUtil {
  ...
  private String izlazi2(String cSkl, Timestamp dateZ, String parametar) {
    ...
    String otpdod = "";
    boolean specotp = frmParam.getParam("robno", "kprOtp", "N", "Otpremnice staviti u KPR tek kad se prebace u racune (D,N)?").equalsIgnoreCase("D");
    ...
    if (parametar.equals("N")) {
      izl = "UNION select '            ' as CSKLUL,MAX(DOKI.CSKL) as CSKLIZ, MAX(0) as RBR, MAX(DOKI.DATDOK) AS DATDOK, " + "STDOKI.VRDOK AS VRDOK, '' AS KOLND, STDOKI.BRDOK AS BRDOK,  SUM(STDOKI.IRAZ-STDOKI.IRAZ) AS SUMZAD, SUM(STDOKI.IRAZ) AS SUMRAZ " + "from STDOKI, DOKI " + "where STDOKI.CSKL='" + cSkl + "' AND " + util.getDoc("DOKI", "STDOKI") + dokumentiN("STDOKI") + " AND DOKI.STAT_KPR='N' " + otpdod + " AND DOKI." + bitvin + " GROUP BY STDOKI.BRDOK, STDOKI.VRDOK " + "UNION " +
        ...
        izl = "UNION select '            ' as CSKLUL,MAX(DOKI.CSKL) as CSKLIZ, MAX(0) as RBR, MAX(DOKI.DATDOK) AS DATDOK, " + "STDOKI.VRDOK AS VRDOK, '' AS KOLND, STDOKI.BRDOK AS BRDOK, SUM(STDOKI.IPRODSP-STDOKI.IPRODSP) AS SUMZAD, " + "SUM(STDOKI.IPRODSP) AS SUMRAZ " + "from STDOKI, DOKI " + "where STDOKI.CSKL='" + cSkl + "' AND " + util.getDoc("DOKI", "STDOKI") + " AND STDOKI.VRDOK NOT IN ('POS','RAC','PRD','PON','NKU', 'NDO', 'DOS', 'ROT','POD','IZD','OTP','TRE','ZAH')" + " AND DOKI.STAT_KPR='N' " + otpdod + " AND DOKI." + bitvin + " GROUP BY STDOKI.BRDOK, STDOKI.VRDOK " + "UNION " +
      ...
      if (specotp) {
        ...
        izl = "UNION select '            ' as CSKLUL,MAX(DOKI.CSKL) as CSKLIZ, MAX(0) as RBR, MAX(DOKI.DATDOK) AS DATDOK, " + "STDOKI.VRDOK AS VRDOK, '' AS KOLND, STDOKI.BRDOK AS BRDOK, SUM(STDOKI.IPRODSP-STDOKI.IRAZ) AS SUMZAD, " + "SUM(STDOKI.IPRODSP) AS SUMRAZ " + "from STDOKI, DOKI " + "where STDOKI.CSKL='" + cSkl + "' AND " + util.getDoc("DOKI", "STDOKI") + " AND STDOKI.VRDOK NOT IN ('POS','RAC','PRD','PON','NKU', 'NDO', 'DOS', 'ROT','POD','IZD','OTP','TRE','ZAH')" + " AND DOKI.STAT_KPR='N' " + otpdod + " AND " + ucc.not() + " AND DOKI." + bitvin + " GROUP BY STDOKI.BRDOK, STDOKI.VRDOK " + "UNION " +
        " select '            ' as CSKLUL,MAX(DOKI.CSKL) as CSKLIZ, MAX(0) as RBR, MAX(DOKI.DATDOK) AS DATDOK, " + "STDOKI.VRDOK AS VRDOK, '' AS KOLND, STDOKI.BRDOK AS BRDOK, -SUM(STDOKI.UIRAB) AS SUMZAD, " + "SUM(STDOKI.IPRODSP) AS SUMRAZ " + "from STDOKI, DOKI " + "where STDOKI.CSKLART='" + cSkl + "' AND " + util.getDoc("DOKI", "STDOKI") + " AND STDOKI.VRDOK IN ('POS','GRN') " + " AND DOKI.STAT_KPR='N' " + otpdod + " AND " + ucc.not() + " AND DOKI." + bitvin + " GROUP BY STDOKI.CSKL, STDOKI.BRDOK, STDOKI.VRDOK " + "UNION " +
        " select '            ' as CSKLUL,MAX(DOKI.CSKL) as CSKLIZ, MAX(0) as RBR, MAX(DOKI.DATDOK) AS DATDOK, " + "STDOKI.VRDOK AS VRDOK, '' AS KOLND, STDOKI.BRDOK AS BRDOK, -SUM(STDOKI.FMCPRP*STDOKI.KOL) AS SUMZAD, " + "SUM(STDOKI.IPRODSP-STDOKI.IPRODSP) AS SUMRAZ " + "from STDOKI, DOKI " + "where STDOKI.CSKLART='" + cSkl + "' AND " + util.getDoc("DOKI", "STDOKI") + " AND STDOKI.VRDOK IN ('RAC') " + " AND DOKI.STAT_KPR='N' " + otpdod + " AND " + ucc.not() + " AND DOKI." + bitvin + " GROUP BY STDOKI.CSKL, STDOKI.BRDOK, STDOKI.VRDOK " + "UNION " +
        "select '            ' as CSKLUL,MAX(DOKI.CSKL) as CSKLIZ, MAX(0) as RBR, MAX(DOKI.DATDOK) AS DATDOK, STDOKI.VRDOK AS VRDOK, '' AS KOLND, STDOKI.BRDOK AS BRDOK,  -SUM(STDOKI.IRAZ) AS SUMZAD, SUM(STDOKI.IRAZ-STDOKI.IRAZ) AS SUMRAZ " + "from STDOKI, DOKI " + "where STDOKI.CSKL='" + cSkl + "' AND " + util.getDoc("DOKI", "STDOKI") + " AND STDOKI.VRDOK in ('ROT','POD','IZD') AND (STDOKI.veza is null or (STDOKI.veza not like '%POS%')) " + " AND DOKI.STAT_KPR='N' AND DOKI." + bitvin + " GROUP BY STDOKI.BRDOK, STDOKI.VRDOK " + " UNION " +
        "select MAX(MESKLA.CSKLUL) as CSKLUL, MAX(MESKLA.CSKLIZ) as CSKLIZ, MAX(0) as RBR, MAX(MESKLA.DATDOK) AS DATDOK, MESKLA.VRDOK AS VRDOK,  '' AS KOLND, MESKLA.BRDOK AS BRDOK, -SUM(STMESKLA.ZADRAZIZ) AS SUMZAD, SUM(STMESKLA.ZADRAZIZ-STMESKLA.ZADRAZIZ) AS SUMRAZ " + "from STMESKLA, MESKLA " + "where  STMESKLA.VRDOK IN ('MEI','MES') and STMESKLA.CSKLIZ='" + cSkl + "' AND STMESKLA.CSKLUL=MESKLA.CSKLUL AND STMESKLA.CSKLIZ=MESKLA.CSKLIZ AND STMESKLA.GOD=MESKLA.GOD " + " AND STMESKLA.BRDOK=MESKLA.BRDOK AND STMESKLA.VRDOK = MESKLA.VRDOK " + dokumentiE("STMESKLA") + " AND MESKLA.STAT_KPR='N' AND MESKLA." + bitvin + " GROUP BY MESKLA.BRDOK, MESKLA.VRDOK, MESKLA.CSKLUL ";
      } else izl = "UNION select '            ' as CSKLUL,MAX(DOKI.CSKL) as CSKLIZ, MAX(0) as RBR, MAX(DOKI.DATDOK) AS DATDOK, " + "STDOKI.VRDOK AS VRDOK, '' AS KOLND, STDOKI.BRDOK AS BRDOK, SUM(STDOKI.IPRODSP-STDOKI.IRAZ) AS SUMZAD, " + "SUM(STDOKI.IPRODSP) AS SUMRAZ " + "from STDOKI, DOKI " + "where STDOKI.CSKL='" + cSkl + "' AND " + util.getDoc("DOKI", "STDOKI") + " AND STDOKI.VRDOK NOT IN ('POS','RAC','PRD','PON','NKU', 'NDO', 'DOS', 'ROT','POD','IZD','OTP','TRE','ZAH')" + " AND DOKI.STAT_KPR='N' " + otpdod + " AND DOKI." + bitvin + " GROUP BY STDOKI.BRDOK, STDOKI.VRDOK " + "UNION " +
      		" select '            ' as CSKLUL,MAX(DOKI.CSKL) as CSKLIZ, MAX(0) as RBR, MAX(DOKI.DATDOK) AS DATDOK, " + "STDOKI.VRDOK AS VRDOK, '' AS KOLND, STDOKI.BRDOK AS BRDOK, -SUM(STDOKI.UIRAB) AS SUMZAD, " + "SUM(STDOKI.IPRODSP) AS SUMRAZ " + "from STDOKI, DOKI " + "where STDOKI.CSKLART='" + cSkl + "' AND " + util.getDoc("DOKI", "STDOKI") + " AND STDOKI.VRDOK IN ('POS','GRN') " + " AND DOKI.STAT_KPR='N' " + otpdod + " AND DOKI." + bitvin + " GROUP BY STDOKI.CSKL, STDOKI.BRDOK, STDOKI.VRDOK " + "UNION " +
          "select MAX(MESKLA.CSKLUL) as CSKLUL, MAX(MESKLA.CSKLIZ) as CSKLIZ, MAX(0) as RBR, MAX(MESKLA.DATDOK) AS DATDOK, MESKLA.VRDOK AS VRDOK,  '' AS KOLND, MESKLA.BRDOK AS BRDOK, -SUM(STMESKLA.ZADRAZIZ) AS SUMZAD, SUM(STMESKLA.ZADRAZIZ-STMESKLA.ZADRAZIZ) AS SUMRAZ " + "from STMESKLA, MESKLA " + "where  STMESKLA.VRDOK IN ('MEI','MES') and STMESKLA.CSKLIZ='" + cSkl + "' AND STMESKLA.CSKLUL=MESKLA.CSKLUL AND STMESKLA.CSKLIZ=MESKLA.CSKLIZ AND STMESKLA.GOD=MESKLA.GOD " + " AND STMESKLA.BRDOK=MESKLA.BRDOK AND STMESKLA.VRDOK = MESKLA.VRDOK " + dokumentiE("STMESKLA") + " AND MESKLA.STAT_KPR='N' AND MESKLA." + bitvin + " GROUP BY MESKLA.BRDOK, MESKLA.VRDOK, MESKLA.CSKLUL " + "UNION " + dodaciIzlaza2(cSkl, dateZ);
    }
    return izl;
  ...
  public String getKarticaArtikla(String cart, String vrzal, String cSkl, String docu, String doci, String newDateZ, String lot, boolean naru, boolean nari) {
    ...
    String qStr =
    // Ulazi
    "SELECT 'A' as ui, 'B' as SRT, DOKU.VRDOK, DOKU.BRDOK, STDOKU.RBR, DOKU.DATDOK, STDOKU.KOL AS KOLUL, (STDOKU.KOL-STDOKU.KOL) AS KOLIZ, " + "STDOKU.ZC, STDOKU.IZAD AS KOLZAD, (STDOKU.IZAD-STDOKU.IZAD) AS KOLRAZ, stdoku.lot, doku.cskl as sklul, '            ' as skliz " + "from STDOKU,DOKU " + "where STDOKU.CART=" + cart + " AND STDOKU.CSKL='" + cSkl + "' AND " + docu + "  AND DOKU.VRDOK NOT IN ('POR','KAL') " + "AND DOKU.DATDOK <= " + newDateZ + ugod + ulot + "UNION ALL " +
    // Samo poravnanja na ulazu
        "SELECT 'A' as ui, 'B' as SRT,  DOKU.VRDOK, DOKU.BRDOK, STDOKU.RBR, DOKU.DATDOK, (STDOKU.KOL-STDOKU.KOL) AS KOLUL, (STDOKU.KOL-STDOKU.KOL) AS KOLIZ, " + this.getZCPor("STDOKU", vrzal) + " STDOKU.PORAV AS KOLZAD, (STDOKU.PORAV-STDOKU.PORAV) AS KOLRAZ, stdoku.lot,'            ' as sklul, '            ' as skliz " + "from STDOKU,DOKU " + "where STDOKU.CART=" + cart + " AND STDOKU.CSKL='" + cSkl + "' AND " + docu + "  AND DOKU.VRDOK='POR' AND STDOKU.SKOL>0 " + "AND DOKU.DATDOK <= " + newDateZ + ugod + ulot + "UNION ALL " +
        // Izlazi
        "SELECT 'B' as ui, 'B' as SRT, DOKI.VRDOK, DOKI.BRDOK, STDOKI.RBR, DOKI.DATDOK, (STDOKI.KOL-STDOKI.KOL) AS KOLUL, STDOKI.KOL AS KOLIZ, " + "STDOKI.ZC, (STDOKI.IRAZ-STDOKI.IRAZ) AS KOLZAD, STDOKI.IRAZ AS KOLRAZ, stdoki.lot,'            ' as sklul, doki.cskl as skliz " + "from STDOKI,DOKI " + "where STDOKI.CART=" + cart + " AND STDOKI.CSKL='" + cSkl + "' AND " + doci + igod + ilot + "AND DOKI.DATDOK <= " + newDateZ + " AND DOKI.VRDOK NOT IN " +dokiF(naru, nari)+ " UNION ALL " +
        // Poravnanje
        "SELECT 'A' as ui, 'A' as SRT, 'POR', DOKU.BRDOK, STDOKU.RBR, DOKU.DATDOK, (STDOKU.KOL-STDOKU.KOL) AS KOLUL, (STDOKU.KOL-STDOKU.KOL) AS KOLIZ, " + this.getZCPor("STDOKU", vrzal) + " STDOKU.PORAV AS KOLZAD, (STDOKU.PORAV-STDOKU.PORAV) AS KOLRAZ, stdoku.lot, '            ' as sklul, '            ' as skliz " + "from STDOKU,DOKU " + "where STDOKU.CART=" + cart + " AND STDOKU.CSKL='" + cSkl + "' AND " + docu + " AND STDOKU.PORAV<>0 " + ugod + ulot + "AND DOKU.DATDOK <= " + newDateZ + " AND STDOKU.VRDOK NOT IN ('POR') " + "UNION ALL " +
        // Medjuskladisnice - ulaz // srt je bilo 'B'
        "SELECT 'A' as ui, 'A' as SRT, MESKLA.VRDOK, MESKLA.BRDOK, STMESKLA.RBR, MESKLA.DATDOK, STMESKLA.KOL AS KOLUL, " + "(STMESKLA.KOL-STMESKLA.KOL) AS KOLIZ, STMESKLA.ZCUL, STMESKLA.ZADRAZUL AS KOLZAD, (STMESKLA.ZADRAZUL-STMESKLA.ZADRAZUL) AS KOLRAZ, stmeskla.lot, meskla.csklul as sklul, meskla.cskliz as skliz " + "from STMESKLA,MESKLA " + "where (MESKLA.VRDOK='MEU' OR MESKLA.VRDOK='MES') AND STMESKLA.CART=" + cart + " AND STMESKLA.CSKLUL='" + cSkl + "' AND MESKLA.CSKLUL=STMESKLA.CSKLUL  " + "AND MESKLA.CSKLIZ=STMESKLA.CSKLIZ  AND MESKLA.GOD=STMESKLA.GOD AND MESKLA.BRDOK=STMESKLA.BRDOK " + " AND MESKLA.VRDOK = STMESKLA.VRDOK AND MESKLA.DATDOK <= " + newDateZ + mgod + mlot + "UNION ALL " +
        // Medjuskladisnice - izlaz
        "SELECT 'B' as ui, 'B' as SRT, MESKLA.VRDOK, MESKLA.BRDOK, STMESKLA.RBR, MESKLA.DATDOK, (STMESKLA.KOL-STMESKLA.KOL) AS KOLUL, " + "STMESKLA.KOL AS KOLIZ, STMESKLA.ZC, (STMESKLA.ZADRAZIZ-STMESKLA.ZADRAZIZ) AS KOLZAD, STMESKLA.ZADRAZIZ AS KOLRAZ, stmeskla.lot, meskla.csklul as sklul, meskla.cskliz as skliz " + "from STMESKLA,MESKLA " + "where (MESKLA.VRDOK='MEI' OR MESKLA.VRDOK='MES') AND STMESKLA.CART=" + cart + " AND STMESKLA.CSKLIZ='" + cSkl + "' AND MESKLA.CSKLIZ=STMESKLA.CSKLIZ " + "AND MESKLA.CSKLUL=STMESKLA.CSKLUL AND MESKLA.GOD=STMESKLA.GOD AND MESKLA.BRDOK=STMESKLA.BRDOK " + " AND MESKLA.VRDOK = STMESKLA.VRDOK AND MESKLA.DATDOK <= " + newDateZ + mgod + mlot + "UNION ALL " +
        // Medjuskladisnice - Poravnanje
        "SELECT 'A' as ui, 'A' as SRT, 'POR', MESKLA.BRDOK, STMESKLA.RBR, MESKLA.DATDOK, (STMESKLA.KOL-STMESKLA.KOL) as KOLUL, (STMESKLA.KOL-STMESKLA.KOL) as KOLIZ," + this.getZCPor("STMESKLA", vrzal) + " STMESKLA.PORAV AS KOLZAD, (STMESKLA.PORAV-STMESKLA.PORAV) AS KOLRAZ, stmeskla.lot, '            ' as sklul, '            ' as skliz " + "from STMESKLA,MESKLA " + "where STMESKLA.CART=" + cart + " AND STMESKLA.CSKLUL='" + cSkl + "' AND MESKLA.CSKLUL=STMESKLA.CSKLUL " + "AND MESKLA.CSKLIZ=STMESKLA.CSKLIZ  AND MESKLA.VRDOK=STMESKLA.VRDOK AND MESKLA.GOD=STMESKLA.GOD " + "AND MESKLA.BRDOK=STMESKLA.BRDOK AND STMESKLA.PORAV<>0 " + "AND MESKLA.DATDOK <= " + newDateZ + mgod + mlot + " ORDER BY 6,2,1,4,5"; // bilo
    // 4,5,2,1
