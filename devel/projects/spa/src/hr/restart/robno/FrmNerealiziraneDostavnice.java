/****license*****************************************************************
**   file: FrmNerealiziraneDostavnice.java
...
public class FrmNerealiziraneDostavnice extends raUpitFat {
  ...
  public FrmNerealiziraneDostavnice() {
    System.out.println("ALIVE!!!"); //XDEBUG delete when no more needed
    try {
      init();
  ...
  public void okPress() {
    ...
    finalSet.setColumns(new Column[] {(Column) dm.getDoki().getColumn("CSKL").clone(),
        							  ...
        							  dm.createIntColumn("BNSP","Nerealizirano stavki")});
    finalSet.open();
  ...
  private String upitString(){
    ...
    "AND (stdoki.veza is null OR stdoki.veza = '') "+
    "AND doki.vrdok = 'DOS' "+
  ...
  public void jptv_doubleClick() {
    ...
    raMasterDetail.showRecord("hr.restart.robno.raDOS", 
