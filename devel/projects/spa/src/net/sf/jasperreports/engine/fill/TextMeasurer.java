...
public class TextMeasurer
{
    ...
    private void initialize(int availableStretchHeight)
    {
        ...
        if (textElement instanceof JRTextField)
          wrapAllowed = ((JRTextField) textElement).isWrapAllowed();
    }
        ...
        )
    {
        ...
        LineBreakMeasurer lineMeasurer = new LineBreakMeasurer(paragraph, 
            wrapAllowed ? BreakIterator.getLineInstance() : BreakIterator.getCharacterInstance(),
                FONT_RENDER_CONTEXT);
        ...
        while (lineMeasurer.getPosition() < paragraph.getEndIndex() && !isMaxHeightReached)
        {
            ...
            if (textHeight + layout.getDescent() <= maxHeight)   
            {
                ...
                if (!wrapAllowed) {
                  positionWithinParagraph = paragraph.getEndIndex();
                  break;
                }
